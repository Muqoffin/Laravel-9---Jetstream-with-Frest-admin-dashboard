            ...
            <div class="card" x-data="{ recovery: false }">
                <div class="card-body">
                    ...
                    <div class="app-brand justify-content-center">
                        <a href="{{ env('APP_URL') }}" class="app-brand-link gap-2">
                    ...
                    <p class="text-start mb-4" x-show="! recovery">
                        {{ __('Please confirm access to your account by entering the authentication code provided by your authenticator application.') }}
                    </p>
                    <p class="text-start mb-4" x-show="recovery">
                        {{ __('Please confirm access to your account by entering one of your emergency recovery codes.') }}
                    </p>
                    <p x-show="! recovery" class="mb-0 fw-semibold">{{ __('Code') }}</p>
                    <p x-show="recovery" class="mb-0 fw-semibold">{{ __('Recovery Code') }}</p>
                            ...
                            <div
                                ...
                                @for ($i = 1; $i < 7; $i++)
                                    <input type="text"
                                        ...
                                        maxlength="1" name="code[{{ $i }}]"
                                        {{ $i == 1 ? 'x-ref="code" autofocus' : '' }}>
                                @endfor
                            </div>
                            <!-- Create a hidden field which is combined by 3 fields above -->
                            <input type="hidden" id="otp" name="code" inputmode="numeric" value=""
                                autocomplete="one-time-code" />
                        </div>

                        <div class="mb-3" x-show="recovery">
                            <input type="text" class="form-control auth-input h-px-50 h-px-50 mx-1 my-2" id="recovery_code"
                                name="recovery_code" autofocus x-ref="recovery_code" autocomplete="one-time-code">
                        </div>
...
@section('custom-js')
    ...
    @if ($errors->any())
        <script>
            $(document).ready(function() {
                toastr.error('{{ $errors->first() }}', 'Notification!');
                toastr.options = {
                    "newestOnTop": true,
                    "progressBar": true,
                    "positionClass": "toast-top-right",
                    ...
                    const keyupHandler = function() {
                        ...
                        numeralMaskList.forEach(numeralMaskEl => {
                            if (numeralMaskEl.value === '') {
                                otpFlag = false;
                                twoStepsForm.querySelector('[id="otp"]').value = '';
                            }
                            otpVal = otpVal + numeralMaskEl.value;
                        });
                        if (otpFlag) {
                            twoStepsForm.querySelector('[id="otp"]').value = otpVal;
                        }
                    };
