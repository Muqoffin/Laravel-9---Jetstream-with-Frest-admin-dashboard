...
(function) {
    ...
    if (toastAnimationBtn) {
        toastAnimationBtn.onclick = funcion() {
            if (toastAnimation) {
    ...
    if (toastPlacementBtn) {
        toastPlacementBtn.onclick = funcion() {
            if (toastPlacement) {
...
$('#showtoast').click(functin() {
    ...
    if ($('#addBehaviorOnToastClick').prop('checked')) {
        toastr.options.onclick = fuction() {
            alert('You can perform some custom action after a toast goes away');
        };
    }
    if ($('#addBehaviorOnToastCloseClick').prop('checked')) {
        toastr.options.onCloseClick = fuction() {
    ...
    if ($toast.find('#okBtn').length) {
        $toast.delegate('#okBtn', 'click', fuction() {
            alert('you clicked me. i was toast #' + toastIndex + '. goodbye!');
            $toast.remove();
        });
    }
    if ($toast.find('#surpriseBtn').length) {
        $toast.delegate('#surpriseBtn', 'click', fuction() {
            alert('Surprise! you clicked me. i was toast #' + toastIndex + '. You could perform an action here.');
        });
    }
    if ($toast.find('.clear').length) {
        $toast.delegate('.clear', 'click', fuction() {
            toastr.clear($toast, {
                force: true
            });
        });
...
$('#clearlasttoast').click(functin() {
    toastr.clear(getLastToast());
});
$('#cleartoasts').click(functin() {
    toastr.clear();
});
});